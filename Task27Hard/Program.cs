...
{
    int count = 0;
    for (double i = number; (i % 10) > 0; i = i * 10) // посчитали цифры справа от запятой
        count++;
    count-- ; // вычли лишний круг счетчика в предыдущем цикле, так как его пройдет даже число без дроби
